...
int count = 0; //counter for background proccesses
...
int main() {
    ...
    for (;;) {
		...
		if (argc == -3) {
            printf("Wrong command with &\n");
            free_memory(&args);
        }

		while(count > 0) {
			pid = wait(&status);
			printf("[%d]+ status: %s\n", pid, WIFEXITED(status) ? "DONE" : "KILLED");
			count--;
		}
...
int command_execution(char *dir, char **symbols, char ***commands) {
    ...
    while (commands != NULL && commands[command_index] != NULL) {
		if (args[0] == NULL) {
		...
		if (strcmp(args[0], "cd") == 0) {   //cd
			...
			if (pid1 == -1) {
				...
				if (symbols != NULL && symbols[symbol_index] != NULL && strcmp(symbols[symbol_index], "<") == 0) { // <, input redirection
				...
				while(wait(NULL) != -1);
				while (symbols[symbol_index] != NULL && (strcmp(symbols[symbol_index], "|") != 0) && (strcmp(symbols[symbol_index], "&") != 0 )) { //redirection of input and output
				...
				if (symbols[symbol_index] != NULL && strcmp(symbols[symbol_index], "&") == 0)
					signal(SIGINT, SIG_IGN);
				printf("OK");
				execvp(args[0], args);
				perror(args[0]);
		...
		while (symbols[symbol_index] != NULL && strcmp(symbols[symbol_index], "|") != 0 && strcmp(symbols[symbol_index], "&") != 0) {
			command_index += 1;
			symbol_index += 1;
		}*/
		while (symbols[symbol_index] != NULL && strcmp(symbols[symbol_index], "&") != 0) { 
...
int read_command(char ***args) {
    ...
    while ((c[0]) != '\n') {
        ...
        while ((c[0] != '&' && special_symbol(c) == 0 && !isspace(c[0])) || quote_flag) {  //single word reading
            if (c[0] == '"') { //quotes
		...
		if (c[0] == '&') { // check if '&' is the last symbol of line
			hang_flag = 1;
		}
        while (special_symbol(c) == 1) {
			...
			if (hang_flag == 1 && c[0] != EOF && c[0] != '\n') { // check if '&' is the last symbol of line
                (*args) = realloc(*args, (arg_number + 1) * sizeof(char **));
                (*args)[arg_number] = NULL;
                while ((c[0] = getchar()) != '\n' && (c[0] != EOF));
                return -3;
